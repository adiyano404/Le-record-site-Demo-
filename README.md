      ...
      const translations = {
        ...
        fr: {
          ...
          about: {
            ...
            footerNote: "Le Record, une plateforme numérique conçue pour devenir, sous l’autorité de l’État guinéen, le registre foncier et immobilier de référence. Aujourd’hui, les informations foncières sont dispersées entre services, souvent sur papier, ce qui favorise les litiges, les doubles ventes et la perte de confiance, tout en privant l’État de recettes importantes. Le Record propose un registre unique et centralisé, où chaque parcelle est identifiée de façon standardisée, rattachée à sa préfecture et à l’une des quatre régions naturelles, avec l’historique des records de transactions.",
        ...
        macroRegions.forEach((mr) => {
          ...
          html += `<div class="bg-slate-900/80 border border-slate-800 rounded-xl p-4 sm:p-5">
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full text-[11px] text-left bg-white text-slate-900 rounded-md overflow-hidden">
                <thead class="bg-slate-100 text-slate-900 border-b border-slate-300">
                  <tr>
                    <th class="py-1.5 px-2 font-semibold">${tCommon.prefectureCode}</th>
                    <th class="py-1.5 px-2 font-semibold">${tCommon.prefecture}</th>
                    <th class="py-1.5 px-2 font-semibold">${tCommon.administrativeRegion}</th>
                    ${admin ? `<th class="py-1.5 px-2"></th>` : ""}
                  </tr>
                </thead>
                <tbody>`;
          prefs
            ...
            .forEach((p) => {
              html += `<tr class="border-b border-slate-200">
                  <td class="py-1.5 px-2">${p.code}</td>
                  <td class="py-1.5 px-2">${p.name}</td>
                  <td class="py-1.5 px-2">${p.administrative_region}</td>
                  ${admin
                    ? `<td class="py-1.5 px-2 text-right"><button type="button" data-pref-code="${p.code}" class="js-edit-pref text-emerald-700 hover:text-emerald-500 underline decoration-emerald-500/70">${tAdmin.editPrefectureButton}</button></td>`
                    : ""}
                </tr>`;
            });
